...
| BR-003 | migration | Shared AppDbContext business access in remaining legacy path(s) | @ivans | Incremental cutover safety | 10-PR2/PR5 | Characterization + cutover verification checklist | in-progress | Incremental slice completed: removed dormant `ReviewsMediatRUnitOfWork` (unused Reviews infrastructure class coupling Reviews to `AppDbContext` + cross-context transaction methods). Validation: `dotnet build src/backend/ECommerce.sln -v minimal` and `dotnet test src/backend/Reviews/ECommerce.Reviews.Tests/ECommerce.Reviews.Tests.csproj` (36 passed, 0 failed). Remaining runtime AppDbContext business paths (for example Payments order repository) still need cutover. |
