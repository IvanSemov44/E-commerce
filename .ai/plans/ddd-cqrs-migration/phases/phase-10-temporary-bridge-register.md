...
| BR-003 | migration | Shared AppDbContext business access in remaining legacy path(s) | @ivans | Incremental cutover safety | 10-PR2/PR5 | Characterization + cutover verification checklist | in-progress | Incremental slices completed: removed dormant `ReviewsMediatRUnitOfWork` (unused Reviews infrastructure class coupling Reviews to `AppDbContext` + cross-context transaction methods) and cut over Payments runtime order read path from shared `AppDbContext` to local `PaymentsDbContext` (`PaymentOrderRepository` + DI/test-host wiring). Validation: `dotnet test src/backend/ECommerce.Tests/ECommerce.Tests.csproj --filter "FullyQualifiedName~Payment"` (44 total, 41 passed, 3 skipped, 0 failed) and `dotnet build src/backend/ECommerce.sln` (succeeded). Remaining shared `AppDbContext` usage is currently at API composition level (startup/migrations + compatibility save coordination) rather than Payments bounded-context runtime repository access. |
