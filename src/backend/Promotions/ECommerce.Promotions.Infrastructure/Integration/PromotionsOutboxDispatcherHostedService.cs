...
/// <summary>
/// Polls promotions.outbox_messages and publishes them to the integration event bus.
/// The Promotions BC owns its full outbox lifecycle independently of the shared dispatcher.
/// </summary>
public sealed class PromotionsOutboxDispatcherHostedService(
    IServiceScopeFactory serviceScopeFactory,
    IOptions<PromotionsOutboxOptions> options,
    ILogger<PromotionsOutboxDispatcherHostedService> logger) : BackgroundService
{
    private static readonly TimeSpan _pollInterval = TimeSpan.FromSeconds(5);
    private readonly PromotionsOutboxOptions _options = options.Value;
    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        ...
        while (!stoppingToken.IsCancellationRequested)
        {
            ...
            {
                logger.LogError(exception, "Promotions outbox dispatch iteration failed");
            }
        ...
        foreach (var message in pendingMessages)
        {
            ...
            catch (Exception exception)
            {
                ...
                if (message.RetryCount >= Math.Max(1, _options.MaxRetryAttempts))
                {
                    ...
                    logger.LogError(
                        exception,
                        "Promotions outbox message {OutboxMessageId} moved to dead-letter after {RetryCount} failed attempts",
                        message.Id,
                        message.RetryCount);
                }
                else
                {
                    ...
                    logger.LogWarning(
                        exception,
                        "Promotions outbox message {OutboxMessageId} failed (attempt {RetryCount}); retry scheduled at {NextAttemptAt}",
                        message.Id,
                        message.RetryCount,
