...
public sealed class InventoryOutboxDispatcherHostedService(
    ...
    IOptions<InventoryOutboxDispatcherOptions> options,
    ILogger<InventoryOutboxDispatcherHostedService> logger) : BackgroundService
{
    ...
    private readonly InventoryOutboxDispatcherOptions _options = options.Value;
    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        ...
        while (!stoppingToken.IsCancellationRequested)
        {
            ...
            {
                logger.LogError(exception, "Inventory outbox dispatch iteration failed");
            }
    ...
    private void HandleMessageFailure(InventoryDbContext dbContext, OutboxMessage message, Exception exception)
    {
        ...
        logger.LogWarning(
            exception,
            "Inventory outbox message {OutboxMessageId} failed (attempt {RetryCount}); retry scheduled at {NextAttemptAt}",
            message.Id,
            message.RetryCount,
            message.NextAttemptAt);
    }

    private void MoveToDeadLetter(InventoryDbContext dbContext, OutboxMessage message, Exception exception)
    {
        ...
        logger.LogError(
            exception,
            "Inventory outbox message {OutboxMessageId} moved to dead-letter after {RetryCount} failed attempts",
            message.Id,
            message.RetryCount);
    }
