...
/// <summary>
/// Polls reviews.outbox_messages and publishes them to the integration event bus.
/// Reviews owns its full outbox lifecycle without touching IntegrationPersistenceDbContext.
/// </summary>
    ...
    ILogger<ReviewsOutboxDispatcherHostedService> logger) : BackgroundService
{
    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        ...
        while (!stoppingToken.IsCancellationRequested)
        {
            ...
            {
                logger.LogError(exception, "Reviews outbox dispatch iteration failed");
            }
        ...
        foreach (var message in pendingMessages)
        {
            ...
            catch (Exception exception)
            {
                ...
                if (message.RetryCount >= Math.Max(1, _options.MaxRetryAttempts))
                {
                    ...
                    logger.LogError(
                        exception,
                        "Reviews outbox message {OutboxMessageId} moved to dead-letter after {RetryCount} failed attempts",
                        message.Id,
                        message.RetryCount);
                }
                else
                {
                    ...
                    logger.LogWarning(
                        exception,
                        "Reviews outbox message {OutboxMessageId} failed (attempt {RetryCount}); retry scheduled at {NextAttemptAt}",
                        message.Id,
                        message.RetryCount,
