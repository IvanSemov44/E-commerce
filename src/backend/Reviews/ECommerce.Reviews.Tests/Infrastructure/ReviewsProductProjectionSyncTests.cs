...
namespace ECommerce.Reviews.Tests.Infrastructure;

[TestClass]
public class ReviewsProductProjectionSyncTests
{
    [TestMethod]
    ...
    public async Task PublishDelete_WhenProjectionExists_RemovesProjection()
    {
        ...
        await publisher.Publish(
            new ProductProjectionUpdatedIntegrationEvent(productId, string.Empty, 0m, true, DateTime.UtcNow),
            CancellationToken.None);
    ...
    public async Task PublishProduct_WhenProjectionExists_UpdatesProjection()
    {
        ...
        db.Products.Add(new ProductReadModel { Id = productId, IsActive = false, UpdatedAt = DateTime.UtcNow.AddDays(-1) });
        await db.SaveChangesAsync();

        var updatedAt = DateTime.UtcNow;
        await publisher.Publish(
            new ProductProjectionUpdatedIntegrationEvent(productId, "Updated Widget", 29.99m, false, updatedAt),
            CancellationToken.None);
