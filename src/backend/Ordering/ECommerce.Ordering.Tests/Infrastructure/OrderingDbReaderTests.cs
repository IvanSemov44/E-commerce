...
[TestClass]
public class OrderingDbReaderTests
{
    [TestMethod]
    public async Task GetProductsAsync_ReturnsRequestedProducts_WithPrimaryImagesOnly()
    {
        ...
        db.Products.AddRange(
            new OrderingProductReadModel { Id = productWithPrimaryImageId, Name = "Primary Image Product", Price = 19.99m, UpdatedAt = DateTime.UtcNow },
            new OrderingProductReadModel { Id = productWithoutPrimaryImageId, Name = "No Primary Image Product", Price = 29.99m, UpdatedAt = DateTime.UtcNow },
            new OrderingProductReadModel { Id = nonRequestedProductId, Name = "Not Requested", Price = 39.99m, UpdatedAt = DateTime.UtcNow });

        db.ProductImages.AddRange(
            new OrderingProductImageReadModel { Id = Guid.NewGuid(), ProductId = productWithPrimaryImageId, Url = "https://cdn.example.com/primary.jpg", IsPrimary = true, UpdatedAt = DateTime.UtcNow },
            new OrderingProductImageReadModel { Id = Guid.NewGuid(), ProductId = productWithPrimaryImageId, Url = "https://cdn.example.com/secondary.jpg", IsPrimary = false, UpdatedAt = DateTime.UtcNow },
            new OrderingProductImageReadModel { Id = Guid.NewGuid(), ProductId = productWithoutPrimaryImageId, Url = "https://cdn.example.com/non-primary.jpg", IsPrimary = false, UpdatedAt = DateTime.UtcNow });
    ...
    public async Task GetShippingAddressAsync_ReturnsAddress_ForMatchingUserAndAddress()
    {
        ...
        {
            Id = addressId, UserId = userId, StreetLine1 = "42 Reader St",
            City = "Testville", Country = "US", PostalCode = "10001", UpdatedAt = DateTime.UtcNow
        });
        await db.SaveChangesAsync();
    ...
    public async Task GetPromoCodeAsync_WhenActivePromoCodeExists_ReturnsDiscountAndId()
    {
        ...
        {
            Id = promoId, Code = "SAVE10", DiscountValue = 10m, IsActive = true, UpdatedAt = DateTime.UtcNow
        });
        await db.SaveChangesAsync();
