...
[TestClass]
public class ProductProjectionSyncTests
{
    [TestMethod]
    ...
    public async Task PublishProductImage_WhenProjectionMissing_InsertsProjection()
    {
        ...
        await publisher.Publish(
            new ProductImageProjectionUpdatedIntegrationEvent(
                imageId, productId, "https://cdn.example.com/p.jpg", true, false, DateTime.UtcNow),
            CancellationToken.None);
    ...
    public async Task PublishDelete_WhenExists_RemovesProductAndImageProjections()
    {
        ...
        db.Products.Add(new ProductReadModel { Id = productId, Name = "To delete", Price = 10m });
        db.ProductImages.Add(new ProductImageReadModel
        {
            Id = imageId, ProductId = productId,
            Url = "https://cdn.example.com/delete.jpg", IsPrimary = true
        });
        await db.SaveChangesAsync();

        await publisher.Publish(
            new ProductImageProjectionUpdatedIntegrationEvent(imageId, productId, "", false, true, DateTime.UtcNow),
            CancellationToken.None);
        await publisher.Publish(
            new ProductProjectionUpdatedIntegrationEvent(productId, "", 0m, true, DateTime.UtcNow),
            CancellationToken.None);
