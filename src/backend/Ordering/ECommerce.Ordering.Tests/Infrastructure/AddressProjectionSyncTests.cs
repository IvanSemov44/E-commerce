...
[TestClass]
public class AddressProjectionSyncTests
{
    [TestMethod]
    public async Task Publish_WhenProjectionMissing_InsertsAddressProjection()
    {
        ...
        var evt = new AddressProjectionUpdatedIntegrationEvent(
            addressId, userId, "42 Reader St", "Testville", "US", "10001", false, DateTime.UtcNow);
    ...
    public async Task Publish_WhenProjectionExists_UpdatesAddressProjection()
    {
        ...
        {
            Id = addressId, UserId = userId, StreetLine1 = "Old Street",
            City = "Old City", Country = "US", PostalCode = "90001"
        });
        await db.SaveChangesAsync();

        var evt = new AddressProjectionUpdatedIntegrationEvent(
            addressId, userId, "New Street", "New City", "US", "10002", false, DateTime.UtcNow);
    ...
    public async Task Publish_WhenDeleted_RemovesAddressProjection()
    {
        ...
        db.Addresses.Add(new AddressReadModel
        {
            Id = addressId, UserId = userId, StreetLine1 = "Delete Street",
            City = "Delete City", Country = "US", PostalCode = "10003"
        });
        await db.SaveChangesAsync();

        var evt = new AddressProjectionUpdatedIntegrationEvent(
            addressId, userId, "Delete Street", "Delete City", "US", "10003", true, DateTime.UtcNow);
