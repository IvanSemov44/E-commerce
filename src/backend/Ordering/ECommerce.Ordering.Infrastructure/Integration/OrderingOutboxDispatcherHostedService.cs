...
public sealed class OrderingOutboxDispatcherHostedService(
    IServiceScopeFactory serviceScopeFactory,
    IOptions<OrderingOutboxDispatcherOptions> options,
    ILogger<OrderingOutboxDispatcherHostedService> logger) : BackgroundService
{
    private static readonly TimeSpan _pollInterval = TimeSpan.FromSeconds(5);
    private readonly OrderingOutboxDispatcherOptions _options = options.Value;
    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        ...
        while (!stoppingToken.IsCancellationRequested)
        {
            ...
            {
                logger.LogError(exception, "Ordering outbox dispatch iteration failed");
            }
    ...
    private void HandleMessageFailure(OrderingDbContext dbContext, OutboxMessage message, Exception exception)
    {
        ...
        logger.LogWarning(
            exception,
            "Ordering outbox message {OutboxMessageId} failed (attempt {RetryCount}); retry scheduled at {NextAttemptAt}",
            message.Id,
            message.RetryCount,
            message.NextAttemptAt);
    }

    private void MoveToDeadLetter(OrderingDbContext dbContext, OutboxMessage message, Exception exception)
    {
        ...
        logger.LogError(
            exception,
            "Ordering outbox message {OutboxMessageId} moved to dead-letter after {RetryCount} failed attempts",
            message.Id,
            message.RetryCount);
    }
