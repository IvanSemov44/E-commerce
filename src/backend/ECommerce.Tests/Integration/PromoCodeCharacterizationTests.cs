...
public class PromoCodeCharacterizationTests
{
    ...
    public async Task GetPromoCodeById_Admin_UnknownId_Returns404()
    {
        ...
        Assert.IsFalse(string.IsNullOrEmpty(content), "Should return some content");
    }
    ...
    {
        using var client = _factory.CreateAdminClient();
        var rawCode = "TEST" + Guid.NewGuid().ToString("N")[..5].ToUpper(System.Globalization.CultureInfo.CurrentCulture);
        var response = await client.PostAsync("/api/promo-codes", BuildCreateBody(rawCode.ToLower(System.Globalization.CultureInfo.CurrentCulture), "Percentage", 10));

        if (response.StatusCode == HttpStatusCode.Created)
        {
            var data = JsonSerializer.Deserialize<JsonElement>(
                await response.Content.ReadAsStringAsync(), _json).GetProperty("data");
            Assert.AreEqual(rawCode.ToUpper(System.Globalization.CultureInfo.CurrentCulture), data.GetProperty("code").GetString());
        }
        else
        {
