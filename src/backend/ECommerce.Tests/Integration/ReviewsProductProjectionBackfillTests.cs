...
[TestClass]
public class ReviewsProductProjectionBackfillTests
{
    [TestMethod]
    ...
    public async Task Backfill_WhenProjectionExists_UpdatesIsActiveWithoutDeletingExtraRows()
    {
        ...
        reviewsDb.Products.Add(new ProductReadModel { Id = productId, IsActive = true, UpdatedAt = DateTime.UtcNow.AddDays(-1) });
        reviewsDb.Products.Add(new ProductReadModel { Id = Guid.NewGuid(), IsActive = true, UpdatedAt = DateTime.UtcNow.AddDays(-1) });
