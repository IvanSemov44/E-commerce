...
public sealed class IdentityOutboxDispatcherHostedService(
    IServiceScopeFactory serviceScopeFactory,
    IOptions<IdentityOutboxDispatcherOptions> options,
    ILogger<IdentityOutboxDispatcherHostedService> logger) : BackgroundService
{
    private static readonly TimeSpan _pollInterval = TimeSpan.FromSeconds(5);
    private readonly IdentityOutboxDispatcherOptions _options = options.Value;
    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        ...
        while (!stoppingToken.IsCancellationRequested)
        {
            ...
            {
                logger.LogError(exception, "Identity outbox dispatch iteration failed");
            }
    ...
    private void HandleMessageFailure(IdentityDbContext dbContext, OutboxMessage message, Exception exception)
    {
        ...
        logger.LogWarning(
            exception,
            "Identity outbox message {OutboxMessageId} failed (attempt {RetryCount}); retry scheduled at {NextAttemptAt}",
            message.Id,
            message.RetryCount,
            message.NextAttemptAt);
    }

    private void MoveToDeadLetter(IdentityDbContext dbContext, OutboxMessage message, Exception exception)
    {
        ...
        logger.LogError(
            exception,
            "Identity outbox message {OutboxMessageId} moved to dead-letter after {RetryCount} failed attempts",
            message.Id,
            message.RetryCount);
    }
