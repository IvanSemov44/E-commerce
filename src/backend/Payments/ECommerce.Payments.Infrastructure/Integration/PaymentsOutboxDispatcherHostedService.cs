...
public sealed class PaymentsOutboxDispatcherHostedService(
    IServiceScopeFactory serviceScopeFactory,
    IOptions<PaymentsOutboxDispatcherOptions> options,
    ILogger<PaymentsOutboxDispatcherHostedService> logger) : BackgroundService
{
    private static readonly TimeSpan _pollInterval = TimeSpan.FromSeconds(5);
    private readonly PaymentsOutboxDispatcherOptions _options = options.Value;
    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        ...
        while (!stoppingToken.IsCancellationRequested)
        {
            ...
            {
                logger.LogError(exception, "Payments outbox dispatch iteration failed");
            }
    ...
    private void HandleMessageFailure(PaymentsDbContext dbContext, OutboxMessage message, Exception exception)
    {
        ...
        if (message.RetryCount >= Math.Max(1, _options.MaxRetryAttempts))
        {
            ...
            logger.LogError(
                exception,
                "Payments outbox message {OutboxMessageId} moved to dead-letter after {RetryCount} failed attempts",
                message.Id, message.RetryCount);
            return;
        }

        var delay = ComputeBackoffDelaySeconds(message.RetryCount);
        message.NextAttemptAt = DateTime.UtcNow.AddSeconds(delay);
        logger.LogWarning(
            exception,
            "Payments outbox message {OutboxMessageId} failed (attempt {RetryCount}); retry at {NextAttemptAt}",
    ...
    private int ComputeBackoffDelaySeconds(int retryCount)
    {
        ...
        return (int)Math.Min(maxDelay, baseDelay * Math.Pow(2, safeRetry - 1));
    }
}
