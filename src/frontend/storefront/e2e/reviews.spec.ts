...
test.describe('Reviews', () => {
  ...
  test('should display review date', async ({ page }) => {
    ...
    if (await productLink.count() > 0) {
      ...
      if (await reviewItem.count() > 0) {
        // Look for date
        const dateElement = reviewItem.locator('[class*="date"], :text(/\\d{1,2}[\\/\\-]\\d{1,2}[\\/\\-]\\d{2,4}/)');
    ...
    if (await productLink.count() > 0) {
      ...
      if (await writeReviewButton.count() > 0) {
        ...
        if (await reviewForm.count() > 0) {
          ...
          const starRating = page.locator('[class*="star-rating"] button, [data-testid*="star"]').first();
  ...
  test('should show helpful vote buttons on reviews', async ({ page }) => {
    ...
    if (await productLink.count() > 0) {
      ...
      if (await reviewItem.count() > 0) {
        ...
        const helpfulButton = reviewItem.locator('button:has-text("Helpful"), button:has-text("Helpful?")');
