  ...
  test.describe('Keyboard Navigation', () => {
    ...
    test('should be able to close modals with Escape key', async ({ page }) => {
      ...
      const modalTrigger = page
        .locator('[data-testid="search-button"], [aria-haspopup="dialog"]')
        .first();
    ...
    test('should trap focus in modals', async ({ page }) => {
      ...
      if ((await modalTrigger.count()) > 0) {
        ...
        const tabbableElements = modal.locator(
        ...
        const isInModal = await focusedElement.evaluate(
          (el, modalEl) => (modalEl ? modalEl.contains(el) : false),
          modalHandle
        );
  ...
  test.describe('Focus Management', () => {
    ...
    test('skip link should be present', async ({ page }) => {
      ...
      if ((await skipLink.count()) > 0) {
        ...
        const isSkipLink = await focusedElement.evaluate(
          (el) => el.getAttribute('href') === '#main' || el.getAttribute('href') === '#content'
        );
