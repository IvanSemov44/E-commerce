...
test.describe('Wishlist', () => {
  ...
  test('should show empty state when wishlist is empty', async ({ page }) => {
    ...
    const emptyState = page.locator('[class*="empty"], :text("No items"), :text("Your wishlist is empty")');
    
    // Either shows empty state or wishlist items
    const hasEmptyState = await emptyState.count() > 0;
    const hasItems = await page.locator('[class*="wishlist-item"], [data-testid="wishlist-item"]').count() > 0;
  ...
  test('should remove item from wishlist', async ({ page }) => {
    ...
    const wishlistItem = page.locator('[class*="wishlist-item"], [data-testid="wishlist-item"]').first();
    
    if (await wishlistItem.count() > 0) {
      // Look for remove button
      const removeButton = wishlistItem.locator('button:has-text("Remove"), button[aria-label*="remove" i]').first();
      ...
      if (await removeButton.count() > 0) {
        ...
        const itemsAfter = await page.locator('[class*="wishlist-item"], [data-testid="wishlist-item"]').count();
  ...
  test('should move item from wishlist to cart', async ({ page }) => {
    ...
    if (await wishlistItem.count() > 0) {
      ...
      const addToCartButton = wishlistItem.locator(
        'button:has-text("Add to Cart"), button:has-text("Move to Cart")'
      ).first();
  ...
  test('should persist wishlist across page navigations', async ({ page }) => {
    ...
    if (await productLink.count() > 0) {
      ...
      const wishlistButton = page.locator('button:has-text("Wishlist"), [data-testid="add-to-wishlist"]').first();
      ...
      if (await wishlistButton.count() > 0) {
        ...
        const wishlistItem = page.locator('[class*="wishlist-item"], [data-testid="wishlist-item"]');
