...
export class RegisterPage extends BasePage {
  ...
  // Locators
  readonly firstNameInput = (): Locator => this.page.getByTestId('first-name-input').or(
    this.page.locator('input[name*="firstName"], input[name*="first_name"], input[placeholder*="first" i]')
  );
  
  readonly lastNameInput = (): Locator => this.page.getByTestId('last-name-input').or(
    this.page.locator('input[name*="lastName"], input[name*="last_name"], input[placeholder*="last" i]')
  );
  ...
  readonly confirmPasswordInput = (): Locator => this.page.getByTestId('confirm-password-input').or(
    this.page.locator('input[name*="confirm"], input[placeholder*="confirm" i]')
  );
  
  readonly registerButton = (): Locator => this.page.getByTestId('register-button').or(
    this.page.locator('button[type="submit"], button:has-text("Register"), button:has-text("Sign Up")')
  );
  
  readonly loginLink = (): Locator => this.page.getByTestId('login-link').or(
    this.page.locator('a[href*="login"], a:has-text("Login"), a:has-text("Sign In")')
  );
  
  readonly termsCheckbox = (): Locator => this.page.getByTestId('terms-checkbox').or(
    this.page.locator('input[type="checkbox"][name*="terms"]')
  );
  ...
  async expectPasswordMismatchError(): Promise<void> {
    const error = this.page.locator('[data-testid="confirm-password-error"], [class*="password-match"], [class*="mismatch"]');
    await this.expectToBeVisible(error);
  }

  async expectWeakPasswordError(): Promise<void> {
    const error = this.page.locator('[data-testid="password-strength"], [class*="weak"], [class*="strength"]');
    await this.expectToBeVisible(error);
  }
  ...
  async getPasswordStrength(): Promise<string | null> {
    const strengthIndicator = this.page.locator('[data-testid="password-strength"], [class*="strength-indicator"]');
    if (await strengthIndicator.count() > 0) {
      return await strengthIndicator.getAttribute('data-strength') || await strengthIndicator.textContent();
    }
    return null;
  }
}
