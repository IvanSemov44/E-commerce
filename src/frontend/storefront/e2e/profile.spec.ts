...
test.describe('Profile Management', () => {
  ...
  test('should redirect to login when accessing profile while not authenticated', async ({ page }) => {
  ...
  test('should display user information on profile page', async ({ page }) => {
    ...
    if (await profileSection.count() > 0) {
      ...
      const hasEmail = await page.locator('input[type="email"], [data-testid="user-email"]').count() > 0;
      const hasName = await page.locator('input[name*="name"], [data-testid="user-name"]').count() > 0;
  ...
  test('should display order history on profile', async ({ page }) => {
    ...
    const orderHistoryLink = page.locator('a[href*="orders"], a:has-text("Orders"), a:has-text("Order History")').first();
  ...
  test('should allow password change', async ({ page }) => {
    ...
    const passwordSection = page.locator('a[href*="password"], button:has-text("Change Password"), a:has-text("Password")').first();
