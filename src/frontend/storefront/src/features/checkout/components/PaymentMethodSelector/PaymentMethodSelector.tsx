...
import { useGetPaymentMethodsQuery } from '../../api/paymentsApi';
...
const METHOD_ICONS: Record<string, React.ReactNode> = {
  ...
  credit_card: <CreditCardIcon aria-hidden="true" className={styles.icon} fill="none" stroke="currentColor" strokeWidth={2} />,
  debit_card:  <DebitCardIcon  aria-hidden="true" className={styles.icon} fill="none" stroke="currentColor" strokeWidth={2} />,
...
const FALLBACK_ICON = (
  <InfoCircleIcon aria-hidden="true" className={styles.icon} fill="none" stroke="currentColor" strokeWidth={2} />
);
...
export default function PaymentMethodSelector({ selectedMethod, onMethodChange }: PaymentMethodSelectorProps) {
  ...
  if (isLoading) {
    return (
      <div className={styles.loading} aria-busy="true" aria-label={t('checkout.selectPaymentMethod')}>
            ...
            >
              ...
              <span className={styles.optionContent}>
                <span className={styles.iconWrapper}>
                  {METHOD_ICONS[method] ?? FALLBACK_ICON}
                </span>
