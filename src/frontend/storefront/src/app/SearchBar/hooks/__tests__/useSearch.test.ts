...
interface MockProductsQueryResult {
  data: { items: Array<{ id: string; name: string; slug: string; price: number }> } | undefined;
  isFetching: boolean;
  error: unknown;
}

// Mock the API
const mockUseGetProductsQuery = vi.fn<() => MockProductsQueryResult>(() => ({
  data: undefined,
  isFetching: false,
  error: null,
}));
...
describe('useSearch', () => {
  ...
  it('should memoize results', () => {
    const mockData = {
      items: [{ id: '1', name: 'Product 1', slug: 'product-1', price: 10 }],
    };
    mockUseGetProductsQuery.mockReturnValue({
      data: mockData,
      isFetching: false,
