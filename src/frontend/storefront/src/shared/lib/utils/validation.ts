...
export const validators = {
  required:
    (fieldName: string = 'This field'): Validator =>
    (value: string) =>
  ...
  numeric: (value: string): string => (/^\d+$/.test(value.trim()) ? '' : 'Must be a number'),
  ...
  url: (value: string): string => (/^https?:\/\/.+\..+/.test(value.trim()) ? '' : 'Invalid URL'),

  match:
    (otherValue: string, fieldName: string = 'Field'): Validator =>
    (value: string) =>
...
export const validateEmail = (value: string): string => validators.email(value);

export const validatePhone = (value: string): string => validators.phone(value);

export const validatePositiveNumber = (value: string): string => validators.positiveNumber(value);
